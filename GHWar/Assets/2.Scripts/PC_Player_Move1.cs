...
public class PC_Player_Move1 : MonoBehaviourPun, IPunObservable
{
    [Header("이동속도")]
    ...
    void Move()
    {
        ...
        else
        {
            ...
            for (int i = 0; i < array_o_VRPlayers.Length; i++)
            {
                PC_Player_Transform.position = Vector3.Lerp(PC_Player_Transform.position, a_v3_setVRpos[i], Time.deltaTime * 20f);
                PC_Player_Transform.rotation = Quaternion.Lerp(PC_Player_Transform.rotation, a_q_setVRrot[i], Time.deltaTime * 20f);
                PC_Player_Transform.position = Vector3.Lerp(PC_Player_Transform.position, a_v3_setPos_handL[i], Time.deltaTime * 20f);
                PC_Player_Transform.rotation = Quaternion.Lerp(PC_Player_Transform.rotation, a_q_setRot_handL[i], Time.deltaTime * 20f);
                PC_Player_Transform.position = Vector3.Lerp(PC_Player_Transform.position, a_v3_setPos_handR[i], Time.deltaTime * 20f);
                PC_Player_Transform.rotation = Quaternion.Lerp(PC_Player_Transform.rotation, a_q_setRot_handR[i], Time.deltaTime * 20f);
            }
        }
    }
